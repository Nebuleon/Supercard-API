...
static inline void jz_flush_dcache(void)
{
	...
	while (start < end) {
		cache_unroll(start,DCIndexWBInv);
...
static inline void jz_writeb(uint32_t address, uint8_t value)
{
	*((volatile uint8_t *)address) = value;
}

static inline void jz_writew(uint32_t address, uint16_t value)
{
	*((volatile uint16_t *)address) = value;
}

static inline void jz_writel(uint32_t address, uint32_t value)
{
	*((volatile uint32_t *)address) = value;
}

static inline uint8_t jz_readb(uint32_t address)
{
	return *((volatile uint8_t *)address);
}

static inline uint16_t jz_readw(uint32_t address)
{
	return *((volatile uint16_t *)address);
}

static inline uint32_t jz_readl(uint32_t address)
{
...
/* Returns the index of the highest (i.e. most significant) bit set in the
 * given 32-bit mask, from 0 to 31, or -1 if the input is 0. */
static inline int _fhs32(uint32_t mask)
{
	int result;
	__asm__ (
		"  clz   %[result], %[mask]\n\t" /* if mask == 0, CLZ yields 32 */
...
/* Returns the highest pending interrupt for the given GPIO group, or -1
 * if there are no pending interrupts in the group. */
static inline int __gpio_group_irq(unsigned int group)
{
	uint32_t group_ipl = REG_GPIO_PXFLG(group);
	return _fhs32(group_ipl);
}
...
{
	static const unsigned char div[] = {1, 2, 3, 4, 6, 8, 12, 16, 24, 32};
